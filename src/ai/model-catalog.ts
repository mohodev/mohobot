...
export interface TaskTuning { primaryCapability: ModelCapability; fallbackCapability?: ModelCapability; temperature:number; background:boolean; }
export const TASK_TUNING: Record<string,TaskTuning>={reply:{primaryCapability:'chat',fallbackCapability:'reasoning',temperature:.8,background:false},vision:{primaryCapability:'vision',fallbackCapability:'ocr',temperature:.2,background:false},embedding:{primaryCapability:'embedding',temperature:0,background:true},rerank:{primaryCapability:'rerank',fallbackCapability:'embedding',temperature:0,background:true},planner:{primaryCapability:'reasoning',fallbackCapability:'chat',temperature:.7,background:true},reflection:{primaryCapability:'chat',temperature:.4,background:true},world:{primaryCapability:'world',fallbackCapability:'reasoning',temperature:.7,background:true},translation:{primaryCapability:'translation',fallbackCapability:'chat',temperature:.2,background:true},safety:{primaryCapability:'safety',fallbackCapability:'chat',temperature:0,background:false}};

export type ModelAvailability='available'|'unavailable'|'open'|'stale'|'unknown';
export interface AvailabilityOverlay{models:Record<string,{availability:ModelAvailability;latencyMs?:number}>;}

/** Capability remains primary; live availability only filters/adjusts equally capable candidates. */
export function recommend(snapshot:ModelCatalogSnapshot,task:string,overlay?:AvailabilityOverlay):CatalogModel[]{const tuning=TASK_TUNING[task];if(!tuning)return[];const availabilityRank:Record<ModelAvailability,number>={available:4,stale:2,unknown:1,unavailable:0,open:-1};return snapshot.models.filter(m=>m.freeEndpoint&&(m.capabilities.includes(tuning.primaryCapability)||(tuning.fallbackCapability!==undefined&&m.capabilities.includes(tuning.fallbackCapability)))&&overlay?.models[m.id]?.availability!=='open').sort((a,b)=>{const capability=Number(b.capabilities.includes(tuning.primaryCapability))-Number(a.capabilities.includes(tuning.primaryCapability));if(capability)return capability;const ar=overlay?.models[a.id]?.availability??'unknown';const br=overlay?.models[b.id]?.availability??'unknown';const availability=availabilityRank[br]-availabilityRank[ar];if(availability)return availability;const al=overlay?.models[a.id]?.latencyMs??Number.POSITIVE_INFINITY;const bl=overlay?.models[b.id]?.latencyMs??Number.POSITIVE_INFINITY;return al-bl||a.id.localeCompare(b.id);});}
