...
export interface ProviderProfile {baseUrl:string;apiKey?:string;model:string;temperature?:number;timeoutMs?:number;retries?:number;budget?:Partial<ModelBudget>;circuit?:{failureThreshold?:number;openMs?:number};}
export interface TaskRoute { primary:string; fallback?:string|readonly string[]; }
...
export class MultiProviderRouter implements AIProvider {
 ...
 constructor(input:{profiles:Record<string,ProviderProfile>;routes:Partial<Record<ModelTask,TaskRoute>>;defaultProfile:string;logger:Logger}){this.#profiles=new Map(Object.entries(input.profiles).map(([id,p])=>{const cfg={provider:'openai-compatible',baseUrl:p.baseUrl,apiKey:p.apiKey??'',model:p.model,temperature:p.temperature??.7,timeoutMs:p.timeoutMs??60_000,retries:p.retries??2,retryBaseDelayMs:500,stream:false,fallbackReply:'',options:{}}satisfies AIConfig;const provider=new BudgetedProvider(new OpenAICompatibleProvider(cfg,{logger:input.logger.child({providerProfile:id})}),p.budget);return[id,{provider,breaker:new CircuitBreaker(p.circuit),model:p.model}]}));this.#routes=input.routes;this.#default=input.defaultProfile;}
 ...
 async chat(messages:ChatMessage[],options:ChatOptions={}):Promise<AIResponse>{const route=this.#routes[options.task??'reply']??{primary:this.#default};const candidates=[route.primary,...(route.fallback===undefined?[]:Array.isArray(route.fallback)?route.fallback:[route.fallback])];let lastError:unknown;for(const id of candidates){try{return await this.#call(id,messages,options);}catch(error){lastError=error;if(!fallbackEligible(error))throw error;}}throw lastError??new Error('no provider route candidates');}
