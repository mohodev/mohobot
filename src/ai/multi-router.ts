...
export class MultiProviderRouter implements AIProvider {
 ...
 async probeAll():Promise<Array<{id:string;ok:boolean;detail?:string;circuit:CircuitSnapshot;checkedAt:number}>>{return Promise.all([...this.#profiles.entries()].map(async([id,r])=>{let result:{ok:boolean;detail?:string};try{result=await r.provider.health();}catch(error){result={ok:false,detail:error instanceof Error?error.message:'probe failed'};}r.lastProbe={...result,checkedAt:Date.now()};return{id,...result,circuit:r.breaker.snapshot(),checkedAt:r.lastProbe.checkedAt};}));}
