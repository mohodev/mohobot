...
describe('OpenAICompatibleProvider', () => {
  ...
  it('never sends max_tokens - output size belongs to the model/gateway',async()=>{let body:Record<string,unknown>={};const fetchImpl=(async(_url:unknown,init:unknown)=>{body=JSON.parse(String((init as RequestInit).body));return jsonResponse(completion('ok'));})as unknown as typeof fetch;const provider=new OpenAICompatibleProvider(cfg(),{logger,fetchImpl});await provider.chat(messages);expect(body).not.toHaveProperty('max_tokens');});
  it('merges free-form options into the request body without clobbering core keys',async()=>{let body:Record<string,unknown>={};const fetchImpl=(async(_url:unknown,init:unknown)=>{body=JSON.parse(String((init as RequestInit).body));return jsonResponse(completion('ok'));})as unknown as typeof fetch;const provider=new OpenAICompatibleProvider(cfg({options:{reasoning:{enabled:false},top_p:.9,model:'hijack-attempt'}}),{logger,fetchImpl});await provider.chat(messages);expect(body.reasoning).toEqual({enabled:false});expect(body.top_p).toBe(.9);expect(body.model).toBe('test-model');});
