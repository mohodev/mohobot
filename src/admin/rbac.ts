...
export type AdminPermission='status.read'|'metrics.read'|'models.read'|'admin.actions.read'|'audit.read'|'characters.read'|'characters.write'|'world.read'|'world.write'|'world.confirm'|'device.read'|'device.write'|'memory.read'|'memory.write'|'plugin.reload'|'config.publish'|'code.write'|'users.read'|'users.create'|'users.update'|'users.disable'|'users.role.assign'|'users.credentials.rotate'|'sessions.read'|'sessions.revoke'|'gateway.read'|'plugin.read'|'runtime.restart'|'sessions.chat.read'|'sessions.chat.delete'|'outbox.read'|'outbox.retry'|'tasks.read';
const READ:AdminPermission[]=['status.read','metrics.read','models.read','characters.read','world.read','device.read','gateway.read','plugin.read','sessions.chat.read','outbox.read','tasks.read'];
const OPERATOR:AdminPermission[]=[...READ,'memory.read','characters.write','world.write','device.write','sessions.chat.delete','outbox.retry'];
const ADMIN:AdminPermission[]=[...OPERATOR,'admin.actions.read','audit.read','memory.write','world.confirm','plugin.reload','runtime.restart','config.publish','users.read','users.create','users.update','users.disable','users.role.assign','users.credentials.rotate','sessions.read','sessions.revoke'];
