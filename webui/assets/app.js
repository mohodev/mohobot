...
async function debugchat(){const caps=await request('/debug/chat/capabilities');const bots=caps.bots||[];app.innerHTML=`<form id="debug-chat-form" class="tool-section"><h2>隔离调试聊天</h2><p class="muted">不访问 Discord、真实会话、世界、设备、记忆、插件或角色 Prompt。</p><label>Bot<select name="botId">${bots.map(b=>`<option value="${esc(b.id)}">${esc(b.name||b.id)} · ${esc(b.model||'')}</option>`).join('')}</select></label><label>测试输入<textarea name="content" rows="6" maxlength="4000" required placeholder="输入模型测试内容"></textarea></label><button>发送隔离请求</button></form><section id="debug-chat-output"></section>`;app.querySelector('#debug-chat-form').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),result=await request('/debug/chat',{method:'POST',body:body({botId:String(f.get('botId')),content:String(f.get('content'))})});document.querySelector('#debug-chat-output').innerHTML=`<section class="tool-section"><h3>模型回复</h3><pre>${esc(result.reply||'')}</pre><div class="muted">${esc(result.trace?.model||'')} · ${esc(result.trace?.latencyMs||0)} ms · ${esc(result.trace?.outcome||'')}</div></section>`;};}
async function knowledge(){const data=await request('/knowledge-bases?limit=100'),bases=data.bases||[];app.innerHTML=`<div class="section-head"><div><h2>本地知识库</h2><p class="muted">纯文本、确定性分块、关键词检索。文档正文不会作为 HTML 渲染。</p></div>${has('knowledge.write')?'<button id="new-kb">新建知识库</button>':''}</div><section id="knowledge-create"></section><div class="table-wrap"><table><thead><tr><th>名称</th><th>描述</th><th>文档</th><th>操作</th></tr></thead><tbody>${bases.map(b=>`<tr><td>${esc(b.name)}</td><td>${esc(b.description||'')}</td><td>${esc(b.documentCount??0)}</td><td><button data-kb="${esc(b.id)}">管理</button></td></tr>`).join('')}</tbody></table></div><section id="knowledge-detail"></section>`;document.querySelector('#new-kb')?.addEventListener('click',()=>{document.querySelector('#knowledge-create').innerHTML=`<form id="kb-form" class="tool-section"><label>名称<input name="name" required maxlength="120"></label><label>描述<textarea name="description" rows="3" maxlength="2000"></textarea></label><button>创建</button></form>`;document.querySelector('#kb-form').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),payload={name:String(f.get('name')),description:String(f.get('description'))};await confirmedRequest('/knowledge-bases',{method:'POST',body:body(payload)},{payload,description:'创建本地知识库'});await knowledge();};});app.querySelectorAll('[data-kb]').forEach(b=>b.onclick=async()=>{const id=b.dataset.kb,base=(await request(`/knowledge-bases/${encodeURIComponent(id)}`)).base,docs=(await request(`/knowledge-bases/${encodeURIComponent(id)}/documents?limit=100`)).documents||[];document.querySelector('#knowledge-detail').innerHTML=`<section class="tool-section"><h3>${esc(base.name)}</h3><form id="kb-doc"><label>文档标题<input name="title" required></label><label>纯文本正文<textarea name="content" rows="9" maxlength="524288" required></textarea></label><button ${has('knowledge.write')?'':'disabled'}>写入文档</button></form><div class="table-wrap"><table><thead><tr><th>标题</th><th>长度</th><th>分块</th></tr></thead><tbody>${docs.map(d=>`<tr><td>${esc(d.title)}</td><td>${esc(d.contentLength)}</td><td>${esc(d.chunkCount)}</td></tr>`).join('')}</tbody></table></div></section>`;document.querySelector('#kb-doc').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),payload={title:String(f.get('title')),content:String(f.get('content'))};await confirmedRequest(`/knowledge-bases/${encodeURIComponent(id)}/documents`,{method:'POST',body:body(payload)},{payload,description:'写入知识库文档'});await knowledge();};});}
...
const tabs = { overview,bots,plugins,extensions,operations,logs,memory,behavior,debugchat,knowledge, users, health, models,providers, config, characters, world,device, metrics };
